...
const Update_Event = ({ eventId,setActiveMenu }) => {
  ...
  const urlToFile = async (url, filename, fileType) => {
    try {
      const response = await fetch(url);
      if (!response.ok) {
        // If image not found (404) or other error, throw
  ...
  const getEventData = async () => {
    try {
      const res = await axios.get(
        `http://localhost:4000/event/fetchOne/${eventId}`,
        {
          withCredentials: true, // 🔑 allow cookies
        }
      );
      setOldImages(res.data.event[0].previous_event_photos);
      const files = await Promise.all(
        res.data.event[0].previous_event_photos.map(async (imgUrl) => {
          try {
            const filename = imgUrl.split("\\").pop().slice(19);
            const fileType = "image/" + filename.split(".").pop();
            const url = `http://localhost:4000/${imgUrl.replaceAll("\\", "/")}`;
            const file = await urlToFile(url, filename, fileType);
            return file;
          } catch (err) {
            console.error("Error converting URL to File:", imgUrl, err);
            return null;
          }
        })
      );
      // Filter out nulls so only successfully fetched images remain
      const validFiles = files.filter(Boolean);
      res.data.event[0].previous_event_photos = validFiles;
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      category_id: event[0].category_id._id,
      venue: event[0].venue,
      state_id: event[0].state_id._id,
      city_id: event[0].city_id._id,
      booking_start_date: event[0].booking_start_date
        ? new Date(event[0].booking_start_date).toISOString().slice(0, 16)
        : "",
      booking_end_date: event[0].booking_end_date
        ? new Date(event[0].booking_end_date).toISOString().slice(0, 16)
        : "",
      event_start_date: event[0].event_start_date
        ? new Date(event[0].event_start_date).toISOString().slice(0, 16)
        : "",
      event_end_date: event[0].event_end_date
        ? new Date(event[0].event_end_date).toISOString().slice(0, 16)
        : "",
      total_tickets: event[0].total_tickets,
      available_tickets: event[0].available_tickets,
      price: event[0].price,
      status: event[0].status,
      previous_photos: event[0].previous_event_photos || [],
    },
    validationSchema,
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if(oldImages.length && oldImages.length>0){
            formData.append("oldImages", oldImages);
        }
        const res = await axios.post(
          `http://localhost:4000/event/update/${eventId}`,
          formData,
          {
            headers: {
  ...
  const renderCreateEventForm = () => (
    ...
    >
      <div className="tw-flex tw-items-center tw-space-x-4 tw-mb-8">
        ...
        >
          <Edit className="tw-w-6 tw-h-6 tw-text-green-400" />
        </motion.div>
        <div>
        ...
        <div className="tw-flex tw-justify-end tw-space-x-4">
          ...
          <motion.div className="tw-flex tw-justify-end tw-pt-6">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              type="button"
              onClick={() => setActiveMenu("view")} // replace with your cancel logic
              className="tw-flex tw-items-center tw-space-x-2 tw-px-8 tw-py-4 tw-bg-red-500 tw-text-white tw-font-bold tw-rounded-2xl hover:tw-bg-red-400 tw-transition-all tw-duration-200 tw-shadow-lg tw-shadow-red-400/20"
            >
